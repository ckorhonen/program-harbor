...
- Published repository and public deployment only if the required external credentials and approvals are available.
...
- Production deployment or paid-service activation without the authorized environment and safe credentials.
...
| C3 | Persistent P0 vertical slice passes locally and in deployed smoke checks | PARTIAL | Local edit/submission/reset/process-restart readback passes; deployment unavailable |
...
- D5: Keep the Next.js App Router choice for the first implementation because it reduces local integration surface in this empty repository; retain adapter boundaries so a Cloudflare Worker/D1 deployment can be tested separately rather than coupling P0 delivery to OpenNext.
...
- Live email, Airtable, Accelevents, R2, and test-email credentials are unavailable. Wrangler account authentication is present, but `wrangler deploy --dry-run` is blocked because this Node/file-backed repository has no Wrangler entry point/configuration or production storage adapter.
